...
def save_creds(username,password,directory,sso):
    file = open('log.txt', 'w')
    file.write(f'{username}%break%{password}%break%{directory}%break%{sso}')
...
if load == 'N' or load == 'n':
    sso = input('Do you need to sign in manually(sso login)?(Y/N): ')
    if sso == 'N' or sso == 'n':
        ...
        if save == 'Y' or save == 'y':
            save_creds(username,password,directory,sso)
...
if sso == 'Y' or sso == 'y':
    while True:
        try:
            done = input("Please do login manually, when you have logged in and are in task page, write 'Done': ")
            break
        except:
            breakpoint
...
while True:
    try:
        ...
        element.send_keys(ep_task(data_questions, data_answers.split(';')[0]))
        time.sleep(speed)
